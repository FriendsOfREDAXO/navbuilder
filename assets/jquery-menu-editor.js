...
(function ($) {
    ...
    $.fn.sortableLists = function (options) {
        ...
        function endDrag(e) {
            ...
            cEl.el.animate({
                    left: offset.left - $(window).scrollLeft() - state.cEl.mL,
                    top: offset.top - $(window).scrollTop() - state.cEl.mT
                }, 250,
...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        ...
        listOptions: {
            hintCss: {border: '1px dashed #13981D'}, opener: {
                as: 'html',
    ...
    function TButtonGroup() {
        ...
        var $btnUp = TButton({
            ...
            text: '<i class="glyphicon glyphicon-chevron-up clickable"></i>'
        });
        var $btnDown = TButton({
            classCss: 'btn btn-default btn-xs btnDown btnMove',
            text: '<i class="glyphicon glyphicon-chevron-down clickable"></i>'
        });
        var $btnOut = TButton({
            classCss: 'btn btn-default btn-xs btnOut btnMove',
            text: '<i class="glyphicon glyphicon-save clickable"></i>'
        });
        var $btnIn = TButton({
            classCss: 'btn btn-default btn-xs btnIn btnMove',
            text: '<i class="glyphicon glyphicon-export clickable"></i>'
        });
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var itemObject = {text: "", href: "", target: "_self", title: "", group: ""};
            var temp = $.extend({}, v);
            if (isParent) {
